...
import { fetchLeadSourceSuccessRatesData } from "@/lib/kpi-data-logic";
import { transformLeadSourceSuccessRateData } from "@/lib/chart-config-utils";
...
export default function SourcePage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (store.leadSourceSuccessRates && store.leadSourceSuccessRatesLastFetch && store.leadSourceSuccessRatesLastFetch > sessionStart) {
        return;
      }
      ...
      try {
        setLoading(true);
        setError(null);
        const response = await fetchLeadSourceSuccessRatesData();
        store.setLeadSourceSuccessRates(response);
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Error fetching lead source success rates data');
      } finally {
        setLoading(false);
      }
    }
    ...
    loadData();
  }, [store.leadSourceSuccessRates, store.leadSourceSuccessRatesLastFetch, store.setLeadSourceSuccessRates]);

  const transformedData = store.leadSourceSuccessRates ? transformLeadSourceSuccessRateData(store.leadSourceSuccessRates) : [];

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-4">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
        <p className="text-muted-foreground">Loading lead source success rates...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div className="space-y-1">
          <h1 className="text-3xl font-bold tracking-tight">Leads by Source Analysis</h1>
          <p className="text-muted-foreground">
            Detailed breakdown of lead source performance and success rates
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {transformedData.map((item) => (
          <Card key={item.leadSource} className="space-y-4">
            <CardHeader className="pb-2">
              <CardTitle className="text-lg">{item.sourceLabel}</CardTitle>
